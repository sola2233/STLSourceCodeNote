...
// 真假的值不应该是个 bool，应该是个有着真假性质的对象，因为需要根据不同的对象激活函数重载机制
// bool 没法做到这一点，以下两个空白的 class 没有任何成员，不会带来额外负担，却又能显示真假
struct __true_type {
};
...
struct __type_traits { 
   ...
   // SGI 把所有内嵌型别都定义为 __false_type，之后再针对每一个标量型别设计适当的特化版本
 ...
 * 提供特化版本。注意，每一个成员的值都是 __true_type，表示这些型别都可以采用最快速
 * 方式（如 memcpy）来进行拷贝（copy）或赋值（assign）操作
 */
